#' Simulates data under an early burst model two regimes, specified in S.matrix
#'
...
sim_multiEB<-function(phylo,pars, verbose=TRUE, min.Nsegments=2500, plot=FALSE,S.matrix,rnd=6, model=c("exponential")){
  ...
  for (p in 1:nrow(pars)){
	...
	test1<-sig2+0.5*(S1*totlen)#find more robust test for this in the future
	test2<-sig2+0.5*(S2*totlen) #find more robust test for this in the future
	if(test1<0 || test2<0){stop("b parameter leads to negative sig2 values")}
	}
    ...
    for(i in 1:phylo$Nnode){
        ...
        for(k in 1:segmentSize[i]){
				...
				tempS1 = ifelse(smat[[timecount]][1,j]>0,S1*(current.time-S1.first.time),0)
				tempS2 = ifelse(smat[[timecount]][2,j]>0,S2*(current.time-S2.first.time),0)
